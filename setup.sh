...
docker-compose up --no-start graph_node

# Start graph-node so we can inspect it
docker-compose start graph_node

# Identify the container ID
CONTAINER_ID=$(docker container ls | grep graph_node | cut -d' ' -f1)
...
function stop_graph_node {
    ...
    docker-compose stop graph_node
}
